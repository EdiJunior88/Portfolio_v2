...
const Avatar = ({ src, alt, name, description }: InterfaceAvatar) => {
  return (
    <div className='container w-11/12 sm:w-full h-auto flex flex-row justify-center gap-2'>
      ...
      <div className='avatar-container-description bg-white w-full h-auto sm:w-96 rounded-lg shadow-lg flex flex-col justify-center items-center gap-2 title-avatar '>
        <p className='avatar-name sm:text-2xl font-bold'>{name}</p>
        <p className='avatar-description sm:text-sm text-[10px] text-gray-500'>{description}</p>
      </div>
    </div>
  );
};
